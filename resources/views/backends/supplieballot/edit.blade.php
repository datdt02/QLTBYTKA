                    ...
                    <div class="row mx-auto">
                        <div class="col-md-12">
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="note"  cols="2"  class="form-control">{{ $ballots->note }}</textarea>
                            </div>
                            <div class="row">
                                <div class="col-md-10">
                                    <div class="form-group">
                                        <select class="form-control select2" id="ballot_device"  name="equipment_id">
                                            ...
                                            @foreach ($eqsupplies as $supplie)
                                                <option value="{{ $supplie->id }}">{{ $supplie->title }} , Serial : {{ $supplie->serial }} , Model : {{ $supplie->model }}, Code : {{ $supplie->code }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <thead>
                                    ...
                                    <tbody class="ballot_device">
                                        @foreach($ballots->supplies as $key => $ballot)
                                            <tr data-id="{{ $ballot->id }}" data-quanlity="{{ $ballot->ballot_amount() }}">
                                                <td>
                                                ...
                                                <td>{{  $ballot->model }}</td>
                                                <td>{{  $ballot->serial }}</td>
                                                <td><input class="quanlity-z" name="amount[]" type="number" min="0" class="form-control" value="{{  $ballot->pivot->amount }}"></td>
                                                <td><input class="currency-z" name="unit_price[]" type="text" class="form-control" value="{{  convert_currency($ballot->pivot->unit_price) }}"></td>
                                                <td class="total">{{   convert_currency($ballot->pivot->amount * $ballot->pivot->unit_price) }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>

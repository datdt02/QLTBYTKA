...
@section('title', __('Danh sách phiếu thiết bị'))
@section('content')
@php
...
<div id="list-events" class="content-wrapper events">
   <section class="content">
      <div class="head container">
         <h1 class="title">{{ __('Danh sách phiếu thiết bị') }}</h1>
      </div>
      <div class="main">
                       ...
                       <tbody class="tbody" action="{{ route('ballot.showEqui') }}">
                            @if(!$ballots->isEmpty())
                                ...
                                <tr>
                                    ...
                                    <td class="text-success btn-ballot" data-id="{{ $ballot->id }}" >{{  $ballot->ballot }}</td>
                                    ...
                                    <td class="text-center">
                                        @if($ballot->status != "public")
                                            <a  title="Cập nhật thiết bị" href="{{ route('ballot.edit',$ballot->id) }}"><i class="fas fa-edit"></i></a>
                                            <a class="text-success ballot-success mr-1 ml-1" title="Duyệt phiếu" data-title="{{ $ballot->ballot }}" data-href="{{ route('ballot.updateSuccess',$ballot->id) }}"><i class="fas fa-check"></i></a>
                                        @endif
                                        <a class="text-danger" title="Xóa thiết bị" href="{{ route('ballot.delete',$ballot->id) }}" data-toggle="modal" data-target="#sideModal" data-direct="modal-top-right"><i class="fas fa-trash"></i></a>
                                    </td>
                               </tr>
                                @endforeach
                            @else

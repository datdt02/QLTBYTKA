...
@section('title', __('Ngoại kiểm thiết bị'))
@section('content')
    @php
        $data_link = [];
        ...
        $data_link['period_of_external_quality_assessment'] = $period_of_external_quality_assessment ?? '';

    @endphp
    <div id="list-events" class="content-wrapper events">
        <section class="content">
            <div class="head container">
                <h1 class="title">{{ __('Danh sách thiết bị cần ngoại kiểm') }}</h1>
                <a href="{{ route('external_quality_assessment.index', $data_link) }}" class="btnprn btn float-right"> <i
            ...
            <div class="main">
                        ...
                        <ul class="nav-filter">
                            ...
                            <li class="">
                                <a class="btn btn-success" style="color: #fff;"
                                   href="{{route("external_quality_assessment.export" , $_GET)}}">
                                    <i class="far fa-file-excel"></i> {{ __('Xuất Excel') }}
                                </a>
                            </li>
                            <li class="">
                                <a class="btn btn-success" style="color: #fff;"
                                   href="{{route("external_quality_assessment.exportNextMonth")}}">
                                    <i class="far fa-file-excel"></i> {{ __('Xuất danh sách cần ngoại kiểm trong tháng tới') }}
                                </a>
                            </li>
                        </ul>
                    </div>
                    <div class="col-md-6 search-form">
                        <form action="{{ route('external_quality_assessment.index') }}" method="GET">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    <select class="form-control select2" name="period_of_external_quality_assessment">
                                        <option value=""> Chọn chu kỳ ngoại kiểm</option>
                                        <option value="0"
                                            {{ $period_of_external_quality_assessment == 0 ? 'selected' : '' }}>Không bắt
                                            buộc
                                        </option>
                                        <option value="12"
                                            {{ $period_of_external_quality_assessment == 12 ? 'selected' : '' }}>12 tháng
                                        </option>
                                        <option value="24"
                                            {{ $period_of_external_quality_assessment == 24 ? 'selected' : '' }}>24 tháng
                                        </option>
                                        <option value="36"
                                            {{ $period_of_external_quality_assessment == 36 ? 'selected' : '' }}>36 tháng
                                        </option>
                                    </select>
                                </div>
                ...
                <ul class="nav-classify">
                    <li {{ ($type_of_inspection ?? "") == "" ? ' class=active' : '' }}><a
                            href="{{ route('external_quality_assessment.index') }}">{{ __('Theo thời gian ngoại kiểm lần cuối') }}</a>
                    </li>
                    <li {{ ($type_of_inspection ?? "") == 'next' ? ' class=active' : '' }}><a
                            href="{{ route('external_quality_assessment.index', ['type_of_inspection' => 'next']) }}">{{ __('Theo thời gian ngoại kiểm tiếp theo') }}</a>
                    </li>
                </ul>
                <div class="card">
                                    ...
                                    @if (!$equipments->isEmpty())
                                        @foreach ($equipments as $index=> $equipment)
                                            <tr>
                                                ...
                                                <td>{{ (isset($equipment->period_of_external_quality_assessment) ? $equipment->period_of_external_quality_assessment : '0') . ' ' . __('tháng') }}
                                                </td>
                                                <td>
                                                    @if ($equipment->last_external_quality_assessment)
                                                        {{$equipment->last_external_quality_assessment}}
                                                    @endif
                                                </td>
                                                <td>
                                                    @if ($equipment->last_external_quality_assessment && $equipment->period_of_external_quality_assessment)
                                                ...
                                                <td class="group-action  text-center">
                                                    ...
                                                    <a href="{{ route('external_quality_assessment.history', $equipment->id) }}"
                                                       title="{{ __('Lịch sử ngoại kiểm') }}"><i
                                                            class="fas fa-history"></i></a>
                                                    <a class="ml-1 mr-1 external_quality_assessment-modal"
                                                       data-title="{{ $equipment->title }}"
                                                       data-href="{{ route('external_quality_assessment.store', $equipment->id) }}"
                                                       title="{{ __('Tạo lịch sử ngoại kiểm') }}"><i
    ...
    <div class="modal fade" id="modal_external_quality_assessment_show" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="external_quality_assessment_show_form" action="" name="frmProducts" class="form-horizontal"
                          method="POST"
                          novalidate="">
                        @csrf
                        <div class="form-group">
                            <label class="control-label">{{ __('Tên thiết bị') }} <small></small></label>
                            <input id="external_quality_assessment_title" type="text" value="" class="form-control" disabled>
                            <div class="help-block with-errors"></div>
                        </div>
                        <div class="form-group">
                            <label class="control-label">{{ __('Thời gian ngoại kiểm') }} <small></small></label>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="content" class="editor form-control"
                                      data-error="{{ __('Vui lòng nhập nội dung ngoại kiểm') }}"

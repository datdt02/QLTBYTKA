...
<div class="content-wrapper">
    <section class="content">
        <div class="container">
            <div class="head">
                <a href="{{ route('protran.index') }}" class="back-icon"><i class="fas fa-angle-left" aria-hidden="true"></i>{{ __('All') }}</a>
                <h1 class="title">{{ __('Phiếu điều chuyển thiết bị') }}</h1>
            </div>
            <div class="main">
                <form action="{{ route('protran.post') }}" data-filter="{{ route('protran.getQuantity') }}" class="dev-form" method="POST" data-toggle="validator" role="form">
                    @csrf
                        ...
                        @include('notices.index')
                                ...
                                <div class="col-md-12">
                                    ...
                                    <option value="">Tất cả thiết bị</option>
                                        @foreach ($equipments as $equipment)
                                            <option value="{{ $equipment->id }}">{{ $equipment->title }} , Model : {{ $equipment->model }} ,Mã hoá TB : {{  $equipment->hash_code }} , Serial : {{ $equipment->serial }} </option>
                                        @endforeach
                                    </select>
                                    </div>
                                </div>
                            ...
                            <div class="group-action">
                                ...
                                <a href="{{ route('protran.index') }}" class="btn btn-secondary">{{ __('Trở về') }}</a>
                            </div>
                        </div>
                </form>
            </div>

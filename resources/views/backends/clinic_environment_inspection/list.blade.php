...
@section('title', __('Kiểm định môi trường phòng thiết bị'))
@section('content')
    @php
        $data_link = [];
        ...
        $data_link['period_of_clinic_environment_inspection'] = $period_of_clinic_environment_inspection ?? '';

    @endphp
    <div id="list-events" class="content-wrapper events">
        <section class="content">
            <div class="head container">
                <h1 class="title">{{ __('Danh sách thiết bị cần kiểm định môi trường phòng') }}</h1>
                <a href="{{ route('clinic_environment_inspection.index', $data_link) }}" class="btnprn btn float-right"> <i
            ...
            <div class="main">
                        ...
                        <ul class="nav-filter">
                            ...
                            <li class="">
                                <a class="btn btn-success" style="color: #fff;"
                                   href="{{route("clinic_environment_inspection.export" , $_GET)}}">
                                    <i class="far fa-file-excel"></i> {{ __('Xuất Excel') }}
                                </a>
                            </li>
                            <li class="">
                                <a class="btn btn-success" style="color: #fff;"
                                   href="{{route("clinic_environment_inspection.exportNextMonth")}}">
                                    <i class="far fa-file-excel"></i> {{ __('Xuất danh sách cần kiểm định môi trường phòng trong tháng tới') }}
                                </a>
                            </li>
                        </ul>
                    </div>
                    <div class="col-md-6 search-form">
                        <form action="{{ route('clinic_environment_inspection.index') }}" method="GET">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    <select class="form-control select2" name="period_of_clinic_environment_inspection">
                                        <option value=""> Chọn chu kỳ kiểm định môi trường phòng</option>
                                        <option value="0"
                                            {{ $period_of_clinic_environment_inspection == 0 ? 'selected' : '' }}>Không bắt
                                            buộc
                                        </option>
                                        <option value="12"
                                            {{ $period_of_clinic_environment_inspection == 12 ? 'selected' : '' }}>12 tháng
                                        </option>
                                        <option value="24"
                                            {{ $period_of_clinic_environment_inspection == 24 ? 'selected' : '' }}>24 tháng
                                        </option>
                                        <option value="36"
                                            {{ $period_of_clinic_environment_inspection == 36 ? 'selected' : '' }}>36 tháng
                                        </option>
                                    </select>
                                </div>
                ...
                <ul class="nav-classify">
                    <li {{ ($type_of_inspection ?? "") == "" ? ' class=active' : '' }}><a
                            href="{{ route('clinic_environment_inspection.index') }}">{{ __('Theo thời gian kiểm định môi trường phòng lần cuối') }}</a>
                    </li>
                    <li {{ ($type_of_inspection ?? "") == 'next' ? ' class=active' : '' }}><a
                            href="{{ route('clinic_environment_inspection.index', ['type_of_inspection' => 'next']) }}">{{ __('Theo thời gian kiểm định môi trường phòng tiếp theo') }}</a>
                    </li>
                </ul>
                <div class="card">
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered" role="table">
                                    ...
                                    <tr>
                                        ...
                                        <th> {{ __('Khoa / phòng') }}</th>
                                        <th>{{ __('Tình trạng') }}</th>
                                        <th> {{ __('Chu kỳ kiểm định môi trường phòng') }}</th>
                                    ...
                                    @if (!$equipments->isEmpty())
                                        @foreach ($equipments as $index=> $equipment)
                                            <tr>
                                                ...
                                                <td>{{ (isset($equipment->period_of_clinic_environment_inspection) ? $equipment->period_of_clinic_environment_inspection : '0') . ' ' . __('tháng') }}
                                                </td>
                                                <td>
                                                    @if ($equipment->last_clinic_environment_inspection)
                                                        {{$equipment->last_clinic_environment_inspection}}
                                                    @endif
                                                </td>
                                                <td>
                                                    @if ($equipment->last_clinic_environment_inspection && $equipment->period_of_clinic_environment_inspection)
                                                ...
                                                <td class="group-action  text-center">
                                                    ...
                                                    <a href="{{ route('clinic_environment_inspection.history', $equipment->id) }}"
                                                       title="{{ __('Lịch sử kiểm định môi trường phòng') }}"><i
                                                            class="fas fa-history"></i></a>
                                                    <a class="ml-1 mr-1 clinic_environment_inspection-modal"
                                                       data-title="{{ $equipment->title }}"
                                                       data-href="{{ route('clinic_environment_inspection.store', $equipment->id) }}"
                                                       title="{{ __('Tạo lịch sử kiểm định môi trường phòng') }}"><i
    ...
    <div class="modal fade" id="modal_clinic_environment_inspection_show" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="clinic_environment_inspection_show_form" action="" name="frmProducts" class="form-horizontal"
                          method="POST"
                          novalidate="">
                        @csrf
                        <div class="form-group">
                            <label class="control-label">{{ __('Tên thiết bị') }} <small></small></label>
                            <input id="clinic_environment_inspection_title" type="text" value="" class="form-control" disabled>
                            <div class="help-block with-errors"></div>
                        </div>
                        <div class="form-group">
                            <label class="control-label">{{ __('Thời gian kiểm định môi trường phòng') }} <small></small></label>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="content" class="editor form-control"
                                      data-error="{{ __('Vui lòng nhập nội dung kiểm định môi trường phòng') }}"

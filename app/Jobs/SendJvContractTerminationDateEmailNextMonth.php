...
class SendJvContractTerminationDateEmailNextMonth implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $export = new EquipmentStatisticsByJvContractExport(
            Carbon::now()->format("Y-m-d"), '', '',
        );
        $fileName = 'Danh sách thiết bị hết hạn hợp đồng LDLK trong tháng ' . $timeInspectionString . '.xlsx';
        ...
        foreach ($departments as $department) {
            $export = new EquipmentStatisticsByJvContractExport(
                Carbon::now()->format("Y-m-d"), '', $department->id
            );
            if (checkIfExportCollectionIsEmpty($export)) {
                unset($export);
            }
    ...
    protected function sendEmails($data, $fileName, $attachments, $timeInspectionInString)
    {
        Mail::send(
            ...
            function ($message) use ($data, $fileName, $attachments, $timeInspectionInString) {
                $message->to($data['email'])->from($data['from'], '[Hết hạn hợp đồng LDLK] ')->subject(
                        'Phòng VTYT thông báo thiết bị hết hạn hợp đồng LDLK thiết bị trong tháng ' . $timeInspectionInString
                    );
                if (is_array($attachments)) {
                    foreach ($attachments as $name => $attachment) {
                        $name =
                            $name . '_Danh sách thiết bị hết hạn hợp đồng LDLK trong tháng ' . $timeInspectionInString . '.xlsx';

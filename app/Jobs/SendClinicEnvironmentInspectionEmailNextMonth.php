...
class SendClinicEnvironmentInspectionEmailNextMonth implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $export = new ClinicEnvironmentInspectionExport(
            'next', $timeInspectionString, '', '', ''
        );
        $fileName = 'Danh sách thiết bị cần kiểm định môi trường phòng trong tháng ' . $timeInspectionString . '.xlsx';
    ...
    protected function sendEmails($data, $fileName, $attachments, $timeInspectionInString)
    {
        Mail::send(
            ...
            function ($message) use ($data, $fileName, $attachments, $timeInspectionInString) {
                $message->to($data['email'])->from($data['from'], '[Kiểm định môi trường phòng] ')->subject(
                        'Phòng VTYT lên lịch kiểm định môi trường phòng thiết bị trong tháng ' . $timeInspectionInString
                    );
                if (is_array($attachments)) {
                    foreach ($attachments as $name => $attachment) {
                        $name =
                            $name
                            . '_Danh sách thiết bị cần kiểm định môi trường phòng trong tháng '

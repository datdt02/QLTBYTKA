...
class SendRadiationInspectionEmailNextMonth implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $export = new RadiationInspectionExport(
            'next', $timeInspectionString, '', '', ''
        );
        $fileName = 'Danh sách thiết bị cần kiểm xạ trong tháng ' . $timeInspectionString . '.xlsx';
    ...
    protected function sendEmails($data, $fileName, $attachments, $timeInspectionInString)
    {
        Mail::send(
            ...
            function ($message) use ($data, $fileName, $attachments, $timeInspectionInString) {
                $message->to($data['email'])->from($data['from'], '[Kiểm xạ] ')->subject(
                        'Phòng VTYT lên lịch kiểm xạ thiết bị trong tháng ' . $timeInspectionInString
                    );
                if (is_array($attachments)) {
                    foreach ($attachments as $name => $attachment) {
                        $name = $name . '_Danh sách thiết bị cần kiểm xạ trong tháng ' . $timeInspectionInString . '.xlsx';

...
class SendLicenseRenewalOfRadiationWorkEmailNextMonth implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $export = new LicenseRenewalOfRadiationWorkExport(
            'next', $timeInspectionString, '', '', ''
        );
        $fileName = 'Danh sách thiết bị cần gia hạn tiến hành CV bức xạ trong tháng ' . $timeInspectionString . '.xlsx';
    ...
    protected function sendEmails($data, $fileName, $attachments, $timeInspectionInString)
    {
        Mail::send(
            ...
            function ($message) use ($data, $fileName, $attachments, $timeInspectionInString) {
                $message->to($data['email'])->from($data['from'], '[Gia hạn tiến hành CV bức xạ] ')->subject(
                        'Phòng VTYT lên lịch gia hạn tiến hành CV bức xạ thiết bị trong tháng ' . $timeInspectionInString
                    );
                if (is_array($attachments)) {
                    foreach ($attachments as $name => $attachment) {
                        $name =
                            $name
                            . '_Danh sách thiết bị cần gia hạn tiến hành CV bức xạ trong tháng '
                            . $timeInspectionInString
                            . '.xlsx';

...
class SendMaintenanceEmailNextMonth implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $fileName = 'Danh sách thiết bị cần bảo dưỡng trong tháng ' . $timeInspectionString . '.xlsx';
    ...
    protected function sendEmails($data, $fileName, $attachments, $timeInspectionInString)
    {
        Mail::send(
            ...
            function ($message) use ($data, $fileName, $attachments, $timeInspectionInString) {
                $message->to($data['email'])->from($data['from'], '[Bảo dưỡng] ')->subject(
                        'Phòng VTYT lên lịch bảo dưỡng thiết bị trong tháng ' . $timeInspectionInString
                    );
                if (is_array($attachments)) {
                    foreach ($attachments as $name => $attachment) {
                        $name = $name . '_Danh sách thiết bị cần bảo dưỡng trong tháng ' . $timeInspectionInString . '.xlsx';

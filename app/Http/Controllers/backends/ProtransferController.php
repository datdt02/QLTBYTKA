...
class ProtransferController extends Controller
{
    public function index(Request  $request)
    {
        ...
        if ($user->can('transfer.read')) {
            ...
            if ($keyword != '') {
                $transfers = $transfers->whereHas('transfer_eqproperty', function ($query) use ($keyword) {
    ...
    public function pdf()
    {
        ...
        $pdf = PDF::loadView('backends.protransfer.transfer_pdf', compact('transfers'));
        return $pdf->download('Phiếu điều chuyển thiết bị.pdf');
    }
    public function showPdf($id)
    {
        $transfers = Transfer::findOrFail($id);
        $pdf = PDF::loadView('backends.protransfer.transfer_show_pdf', compact('transfers'));
    ...
    public function wordExport($id)
    {
        ...
        $transfersWord->setValue('department_users_id',  isset($transfers->transfer_department) && isset($transfers->transfer_department->users) ? $transfers->transfer_department->user->name : '');
    ...
    public function create()
    {
        ...
        if ($user->can('create', Transfer::class)) {
            ...
            return view('backends.protransfer.create', compact('equipments', 'departments', 'cur_day'));
        } else {
            abort(403);
        }
    }
    ...
    {
        $equipment = Eqproperty::findOrFail($request->equipment_id);
        $rules = [
        ...
        if ($validator->fails()) :
            ...
            if ($transfers) {
                return redirect()->route('protran.index')->with('success', 'Đã gửi phiếu điều chuyển thiết bị ' . $transfers->transfer_eqproperty->title . ' đến ' . $transfers->transfer_department->title . '');
            } else {
    ...
    public function edit($id, $supplies_id)
    {
        ...
        if ($user->can('transfer.approved')) {
            ...
            $departments = Department::select('id', 'title')->where('id', '!=', $user->department_id)->get();
            $cur_day = Carbon::now()->format('Y-m-d');
            return view('backends.protransfer.edit', compact('equipments', 'cur_day', 'departments', 'transfers', 'equipments_supplies'));
        } else {
            abort(403);
        }
    }
    public function update(Request  $request, $id)
    {
        ...
        if ($request->status == "public") {
            ...
            if (intval($equipments->amount) > intval($transfers->amount)) {
                ...
                $equipments->save();
                $equipments_v2 = Eqproperty::where('parent_id', $equipments->id)->where('department_id', $transfers->department_id)->first();
                if ($equipments_v2) {
    ...
    public function destroy($id)
    {
        ...
        if ($user->can('delete', $transfers)) {
            ...
            \DB::table('notifications')
                ...
                ->delete();
            return redirect()->route('protran.index')->with('success', 'Xóa thành công');
        } else {
            abort(403);
        }
    }
    ...
    {
        $equipment = Eqproperty::with('equipment_department')->select('id', 'title', 'amount', 'department_id')->where('id', $request->id)->first();

...
class ProRepairController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($user->can('prorepair.read')) {
            ...
            return view('backends.prorepairs.list', $data);
        } else {
            abort(403);
        }
    }
    public function create($equip_id)
    {
        ...
        if ($user->can('prorepair.create')) {
            ...
            return view('backends.prorepairs.create', $data);
        } else {
            abort(403);
        }
    }
    public function store(Request $request, $equip_id)
    {
        ...
        if ($validator->fails()) :
            return redirect()->route('prorepair.create', ['equip_id' => $equip_id])->withErrors($validator)->withInput();
        else :
            ...
            return redirect()->route('prorepair.index');
        endif;
    }
    public function listRepair(Request $request, $equip_id)
    {
        ...
        return view('backends.prorepairs.list-histories', $data);
    }
    ...
    public function edit($equip_id, $repair_id)
    {
        ...
        if ($user->can('prorepair.update')) {
            ...
            return view('backends.prorepairs.edit', $data);
        } else {
            abort(403);
        }
    }
    public function update(Request $request, $equip_id, $repair_id)
    {
        ...
        if ($repair->acceptance != 'accepted') {
            if ($request->acceptance == 'accepted') {
                ...
                //notify
                $array_user = getUserPhcToNotify($equip_id);
                if ($array_user != null) {
                    foreach ($array_user as $key => $value) {
        ...
        return redirect()->route('prorepair.edit', ['equip_id' => $equip_id, 'repair_id' => $repair_id]);
    }

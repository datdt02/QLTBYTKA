...
class EquipmentController extends Controller
{
    ...
    public function show(Request $request, $id)
    {
        ...
        if ($user->can('equipment.show')) {
            ...
            $activities = Activity::with('causer')->where("subject_type", "App\Models\Equipment")
    ...
    public function create()
    {
        ...
        if ($user->can('create', Equipment::class)) {
            ...
            return view('backends.equipments.create', compact(
                'maintenances',
                'providers',
                'repairs',
                'users',
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) :
            ...
            // Xử lí next_inspection, next-maintance
            if ($atribute['last_inspection'] && $atribute['regular_inspection']) {
                $atribute['next_inspection'] = date("Y-m-d", strtotime("+" . $atribute['regular_inspection'] . "months", strtotime($atribute['last_inspection'])));
            }
            if ($atribute['last_maintenance'] && $atribute['regular_maintenance']) {
                $atribute['next_maintenance'] = date("Y-m-d", strtotime("+" . $atribute['regular_maintenance'] . "months", strtotime($atribute['last_maintenance'])));
            }
            ...
            updateAllInspectionDate($equipments, $atribute["last_radiation_inspection"],
                $atribute["periodic_radiation_inspection"],
                $atribute["last_inspection"],
                $atribute["regular_inspection"],
                $atribute["last_maintenance"],
                $atribute["regular_maintenance"]);
            updateInspectionDate($equipments, "external_quality_assessment", $equipments->last_external_quality_assessment, $equipments->period_of_external_quality_assessment);
            updateInspectionDate($equipments, "clinic_environment_inspection", $equipments->last_clinic_environment_inspection, $equipments->period_of_clinic_environment_inspection);
            updateInspectionDate($equipments, "license_renewal_of_radiation_work", $equipments->last_license_renewal_of_radiation_work, $equipments->period_of_license_renewal_of_radiation_work);
    ...
    public function edit($id)
    {
        ...
        if ($user->can('update', $equipments)) {
            ...
            return view('backends.equipments.edit', compact(
                'equipments',
                'maintenances',
                'providers',
                'repairs',
